...
public class SwarmFunctionalBehavior implements Behavior {
    ...
    public static final double WEIGHT_COHESION = 0.88;//0.01;
    ...
    private void computeSwarmAcceleration() {
        ...
        buffer.parallelStream().forEach(agent -> {
            Vektor2D alignment = (Vektor2D) computeAlignment(agent).mult(WEIGHT_ALIGMENT);
            Vektor2D cohesion = (Vektor2D) computeCohesion(agent).mult(WEIGHT_COHESION);
            Vektor2D separation = (Vektor2D) computeSeparation(agent).mult(WEIGHT_SEPARATION);

            Vektor2D desired = (Vektor2D) LinAlg
                    .add(alignment, cohesion, separation)
                    .normalize()
                    .mult(agent.getMaxSpeed());
